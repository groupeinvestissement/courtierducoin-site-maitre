(() => {
  ...
  setOnce('cdc_entry_host', pageEntry?.startsWith('vs-') ? 'vaudreuil-soulanges.courtierducoin.ca' : location.host);
  setOnce('cdc_entry_path', pageEntry?.startsWith('vs-') ? `/${pageEntry.slice(3).replace('universal','')}` : location.pathname);
  ...
  const track = (event, form) => window.dataLayer.push({
    event,
  ...
  forms.forEach((form) => {
    ...
    form.addEventListener('input', () => {
      if (!started) { started = true; track('form_start', form); }
    });
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        status.textContent = 'Votre demande n’a pas pu être envoyée. Vous pouvez réessayer ou appeler/texter Pierre au 514 216-4013.';
        status.className = 'form-status error';
  ...
  document.querySelectorAll('[data-home-video],[data-investor-video],[data-video]').forEach((video) => video.addEventListener('click', () => track('video_start')));

  const getPath = (object, path) => path.split('.').reduce((value, key) => value?.[key], object);
  const renderStat = (element, value) => {
    if (value === null || value === undefined) { element.textContent = 'Non publié — volume insuffisant'; return; }
  ...
  if (document.querySelector('[data-market-stat]')) {
    fetch('/data/vaudreuil-soulanges-market.json', {credentials: 'same-origin'})
