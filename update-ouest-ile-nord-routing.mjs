...
for (const [index, code] of ['', 'o1a11', '02a22', '03i33', '04m44', '05c55'].entries()) {
  const entry = `https://ouest-ile-nord.courtierducoin.ca/${code}`;
  const destination = `${routes[index]}?entry=ouestnord-${code || 'universal'}`;
...
console.log('Updated West Island North sitemap and redirect map.');
