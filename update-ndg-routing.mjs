...
for (const [index, code] of ['', 'o1a11', '02a22', '03i33', '04m44', '05c55'].entries()) {
  const entry = `https://ndg.courtierducoin.ca/${code}`;
  const destination = `${routes[index]}?entry=ndg-${code || 'universal'}`;
...
console.log('Updated NDG–Montréal-Ouest sitemap and redirect map.');
