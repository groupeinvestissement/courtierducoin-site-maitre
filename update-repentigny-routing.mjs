...
for (const [index, code] of ['', 'o1a11', '02a22', '03i33', '04m44', '05c55'].entries()) {
  const entry = `https://repentigny.courtierducoin.ca/${code}`;
  const destination = `${routes[index]}?entry=repentigny-${code || 'universal'}`;
...
console.log('Updated Repentigny sitemap and redirect map.');
